/*********************************************************************
 ...
 This file video_capture.cpp is part of RPiCast project
...
#define MAX_DEFAULT_HEIGHT 1600
#define MAX_DEFAULT_WIDTH 2560
...
void VideoCapture::senddata(unsigned char* data, unsigned int size)
{
    DBG_ALL("ENter %x", data);
...
//    avpicture_fill((AVPicture *) yuv_frame, buffer, AV_PIX_FMT_YUV420P, SCALED_WIDTH, SCALED_HEIGHT);
//    DBG_ALL("Exit1");
//    sws_scale(swsCtx, (const uint8_t * const *) bgr_frame->data, bgr_frame->linesize, 0, 1800,
//        yuv_frame->data, yuv_frame->linesize);
//    DBG_ALL("Exit");
    ...
    AVFrame* bgr_frame;
    struct SwsContext * swsCtx;
    swsCtx = sws_getContext(MAX_DEFAULT_WIDTH, MAX_DEFAULT_HEIGHT, AV_PIX_FMT_YUV420P, SCALED_WIDTH, SCALED_HEIGHT,
            AV_PIX_FMT_YUV420P,
            SWS_FAST_BILINEAR, NULL, NULL, NULL);
    Buffer* buf = m_output->GetBuffer();
    yuv_frame = static_cast<AVFrame*>((void*) buf->GetData());
    avpicture_fill((AVPicture *) yuv_frame, data, AV_PIX_FMT_YUV420P, MAX_DEFAULT_WIDTH, MAX_DEFAULT_HEIGHT);
    //sws_scale(swsCtx, (const uint8_t * const *) bgr_frame->data, bgr_frame->linesize, 0, 1800,
       //yuv_frame->data, yuv_frame->linesize);
